...
class ProductController extends Controller
{
    ...
    public function ProductProduct($id)
    {
        $product = DB::table('products')->where('id', $id)->first();
    ...
    public function UpdateProductPhoto(Request $request, $id)
    {
        ...
        if ($image_two) {
            unlink($oldtwo);
            $image_name = hexdec(uniqid());
            $extension = strtolower($image_two->getClientOriginalExtension());
            $image_full_name = $image_name . '.' . $extension;
            $upload_path = 'public/media/product/';
            $image_url = $upload_path . $image_full_name;
            $success = $image_two->move($upload_path, $image_full_name);
            $data['image_two'] = $image_url;
            $product_image_two = DB::table('products')->where('id', $id)->update($data);
            $notification = array(
        ...
        if ($image_three) {
            unlink($oldthree);
            $image_name = hexdec(uniqid());
            $extension = strtolower($image_three->getClientOriginalExtension());
            $image_full_name = $image_name . '.' . $extension;
            $upload_path = 'public/media/product/';
            $image_url = $upload_path . $image_full_name;
            $success = $image_three->move($upload_path, $image_full_name);
            $data['image_three'] = $image_url;
            $product_image_three = DB::table('products')->where('id', $id)->update($data);
            $notification = array(
